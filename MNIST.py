...
n_hidden_2 = 28*28   # 2nd layer num features
n_hidden_3 = 28*28   # 3rd layer num features
n_hidden_4 = 28*28   # 3rd layer num features
...
#We create a third hidden layer
print("Creating a third hidden layer with ", n_hidden_3, " nodes")
w_layer_3    = tf.Variable(tf.random_normal([n_hidden_2, n_hidden_3]))
bias_layer_3 = tf.Variable(tf.random_normal([n_hidden_3]))
layer_3      = tf.nn.relu(tf.add(tf.matmul(layer_2,w_layer_3),bias_layer_3))
"""
print("Creating a fourth hidden layer with ", n_hidden_4, " nodes")
w_layer_4    = tf.Variable(tf.random_normal([n_hidden_3, n_hidden_4]))
bias_layer_4 = tf.Variable(tf.random_normal([n_hidden_4]))
layer_4      = tf.nn.relu(tf.add(tf.matmul(layer_3,w_layer_4),bias_layer_4))
"""
...
output       = tf.Variable(tf.random_normal([n_hidden_3, n_classes]))
bias_output  = tf.Variable(tf.random_normal([n_classes]))
model  = tf.matmul(layer_3, output) + bias_output
